...
/**
 * Implements the IEmployeeRepository interface using a JSON file for persistence. This class
 * manages the serialization and deserialization of Employee objects.
 */
public class EmployeeRepository implements IEmployeeRepository {
  ...
  public void save(Employee employee) throws DuplicateEntityException {
    if (employees.containsKey(employee.getId())) {
      throw new DuplicateEntityException(
          "Error: Employee with ID " + employee.getId() + " already exists.");
    }
  ...
  /**
   * Deserializes employee data from the JSON file into the in-memory map. If the file does not
  ...
  private void loadFromFile() {
    ...
    try (Reader reader = new FileReader(file)) {
      ...
      System.err.println(
          "Error parsing employee data file. The file might be corrupt: " + e.getMessage());
    }
  }
}
